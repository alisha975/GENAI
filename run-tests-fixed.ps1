...
Write-Host "[OK] Jenkinsfile found" -ForegroundColor Green
...
if ($jenkinsfileContent -match "pipeline\s*\{" -and $jenkinsfileContent -match "agent\s+any") {
    Write-Host "[PASS] Pipeline definition exists" -ForegroundColor Green
    $testResults += @{Test = "Pipeline Definition"; Result = "PASSED" }
} else {
    Write-Host "[FAIL] Pipeline definition not found" -ForegroundColor Red
...
foreach ($envVar in $requiredEnvVars) {
    if ($jenkinsfileContent -match $envVar) {
        if ($Verbose) {
            Write-Host "  [OK] Found: $envVar" -ForegroundColor Green
        }
    } else {
        Write-Host "  [FAIL] Missing: $envVar" -ForegroundColor Red
        $allEnvVarsFound = $false
    }
}

if ($allEnvVarsFound) {
    Write-Host "[PASS] All environment variables found" -ForegroundColor Green
    $testResults += @{Test = "Environment Variables"; Result = "PASSED" }
} else {
    Write-Host "[FAIL] Some environment variables missing" -ForegroundColor Red
...
foreach ($stage in $requiredStages) {
    ...
    if ($jenkinsfileContent -match [regex]::Escape($stagePattern)) {
        if ($Verbose) {
            Write-Host "  [OK] Found: $stage" -ForegroundColor Green
        }
    } else {
        Write-Host "  [FAIL] Missing: $stage" -ForegroundColor Red
        $allStagesFound = $false
    }
}

if ($allStagesFound) {
    Write-Host "[PASS] All required stages found ($($requiredStages.Count) stages)" -ForegroundColor Green
    $testResults += @{Test = "Required Stages"; Result = "PASSED" }
} else {
    Write-Host "[FAIL] Some stages missing" -ForegroundColor Red
...
foreach ($key in $deploymentChecks.Keys) {
    ...
    if ($jenkinsfileContent -match "$key.*$value") {
        if ($Verbose) {
            Write-Host "  [OK] $key = $value" -ForegroundColor Green
        }
    } else {
        Write-Host "  [FAIL] $key configuration incorrect" -ForegroundColor Red
        $allDeploymentConfigOk = $false
    }
}

if ($allDeploymentConfigOk) {
    Write-Host "[PASS] Deployment configuration correct" -ForegroundColor Green
    $testResults += @{Test = "Deployment Configuration"; Result = "PASSED" }
} else {
    Write-Host "[FAIL] Deployment configuration incorrect" -ForegroundColor Red
...
foreach ($check in $sshChecks) {
    if ($jenkinsfileContent -match $check) {
        if ($Verbose) {
            Write-Host "  [OK] Found: $check" -ForegroundColor Green
        }
    } else {
...
if ($allSSHChecksOk) {
    Write-Host "[PASS] SSH configuration correct" -ForegroundColor Green
    $testResults += @{Test = "SSH Configuration"; Result = "PASSED" }
} else {
    Write-Host "[FAIL] SSH configuration incorrect" -ForegroundColor Red
...
if ($allDockerChecksOk) {
    Write-Host "[PASS] All Docker commands present" -ForegroundColor Green
    $testResults += @{Test = "Docker Commands"; Result = "PASSED" }
} else {
    Write-Host "[FAIL] Some Docker commands missing" -ForegroundColor Red
...
if ($allFileTransferChecksOk) {
    Write-Host "[PASS] File transfer mechanisms configured" -ForegroundColor Green
    $testResults += @{Test = "File Transfer"; Result = "PASSED" }
} else {
    Write-Host "[FAIL] File transfer configuration incomplete" -ForegroundColor Red
...
if ($allHealthCheckChecksOk) {
    Write-Host "[PASS] Health check endpoints configured" -ForegroundColor Green
    $testResults += @{Test = "Health Checks"; Result = "PASSED" }
} else {
    Write-Host "[FAIL] Health check configuration incomplete" -ForegroundColor Red
...
foreach ($check in $postActionChecks) {
    if ($jenkinsfileContent -match $check) {
        if ($Verbose) {
            Write-Host "  [OK] Found pattern: $check" -ForegroundColor Green
        }
    } else {
        Write-Host "  [FAIL] Missing pattern: $check" -ForegroundColor Red
        $allPostActionChecksOk = $false
    }
}

if ($allPostActionChecksOk) {
    Write-Host "[PASS] Post-build actions configured" -ForegroundColor Green
    $testResults += @{Test = "Post Actions"; Result = "PASSED" }
} else {
    Write-Host "[FAIL] Post-build actions incomplete" -ForegroundColor Red
...
$testResults | ForEach-Object {
    $status = if ($_.Result -eq "PASSED") { "[PASS]" } else { "[FAIL]" }
